...
class TestAdminUserAPI:
    ...
    def test_get_user(self, client):
        ...
        # 获取用户
        response = client.get('/admin/users/getuser')
        assert response.status_code == 200
        data = response.get_json()
        assert data['success'] is True
...
class TestAdminAPIKeyAPI:
    ...
    def test_create_api_key(self, client):
        ...
        # 创建新 Key
        response = client.post('/admin/users/keyuser/keys', json={
            "name": "test-key",
            "permissions": "stt,tts"
        })
        assert response.status_code == 200
    ...
    def test_list_api_keys(self, client):
        ...
        # 列出 Keys
        response = client.get('/admin/users/listuser/keys')
        assert response.status_code == 200
        data = response.get_json()
        assert data['success'] is True
        assert 'keys' in data
    ...
    def test_revoke_api_key(self, client):
        ...
        # 撤销 Key
        response = client.post(f'/admin/keys/{api_key}/revoke')
        assert response.status_code == 200
        data = response.get_json()
        assert data['success'] is True
